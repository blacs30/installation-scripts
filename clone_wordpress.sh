...
#Request SSH Access
read -r -p "SSH Username (e.g. $default_server_user_source): " server_user_source
server_user_source=${server_user_source:-$default_server_user_source}
echo "$server_user_source"
read -r -p "SSH Server IP (e.g. $default_server_ip_source): " server_ip_source
server_ip_source=${server_ip_source:-$default_server_ip_source}
echo "$server_ip_source"

#Request MySQL Admin
read -r -p "MySQL Master Username (e.g. $default_mysql_user): " mysql_user
mysql_user=${mysql_user:-$default_mysql_user}
echo "$mysql_user"
read -r -p "MySQL Master Password (e.g. $default_mysql_pass): " mysql_pass
mysql_pass=${mysql_pass:-$default_mysql_pass}
echo "$mysql_pass"

# Request Source Settings
read -r -p "Source Domain (e.g. $default_source_domain): " source_domain
source_domain=${source_domain:-$default_source_domain}
echo "$source_domain"
read -r -p "Source Directory (no trailing slash e.g. $default_source_directory): " source_directory
source_directory=${source_directory:-$default_source_directory}
echo "$source_directory"
read -r -p "Source Database Name (e.g. $default_source_dbname): " source_dbname
source_dbname=${source_dbname:-$default_source_dbname}
echo "$source_dbname"
read -r -p "Source Database Host (e.g. $default_source_dbhost): " source_dbhost
source_dbhost=${source_dbhost:-$default_source_dbhost}
echo "$source_dbname"
read -r -p "Source Database User (e.g. $default_source_dbuser): " source_dbuser
source_dbuser=${source_dbuser:-$default_source_dbuser}
echo "$source_dbuser"
read -r -p "Source Database Pass (e.g. $default_source_dbpass): " source_dbpass
source_dbpass=${source_dbpass:-$default_source_dbpass}
echo "$source_dbpass"
read -r -p "Source Database Table Prefix (e.g. $default_source_dbtableprefix): " source_dbtableprefix
source_dbtableprefix=${source_dbtableprefix:-$default_source_dbtableprefix}
echo "$source_dbtableprefix"
# Request Target Settings
read -r -p "Target Domain (e.g. $default_target_domain): " target_domain
target_domain=${target_domain:-$default_target_domain}
echo "$target_domain"
read -r -p "Target Directory (no trailing slash e.g. $default_target_directory): " target_directory
target_directory=${target_directory:-$default_target_directory}
echo "$target_directory"
read -r -p "Target Database Name (max 16 char)(e.g. $default_target_dbname): " target_dbname
target_dbname=${target_dbname:-$default_target_dbname}
echo "$target_dbname"
read -r -p "Target Database User (e.g. $default_target_dbuser): " target_dbuser
target_dbuser=${target_dbuser:-$default_target_dbuser}
echo "$target_dbuser"
read -r -p "Target Database Pass (e.g. $default_target_dbpass): " target_dbpass
target_dbpass=${target_dbpass:-$default_target_dbpass}
echo "$target_dbpass"
read -r -p "Target Database Host (e.g. $default_target_dbhost): " target_dbhost
target_dbhost=${target_dbhost:-$default_target_dbhost}
echo "$target_dbhost"

read -r -p "Tmp directory should exist on both servers (e.g. $default_tmp_dir): " tmp_dir
...
siteCount=$(find /etc/apache2/sites-enabled/* -maxdepth 1 -name '*' | wc -l)

export siteCount
siteCountIncremented=$(printf "%03d" $((siteCount+1)))
export siteCountIncremented
apacheConfigFile=$siteCountIncremented-$target_domain.conf
export apacheConfigFile
SSL_CONF=/etc/apache2/sites-available/$apacheConfigFile
export SSL_CONF

# Check if vhost config already exist for the given domain
CONFIGEXIST=$(find /etc/apache2/sites-available -type f -name "*$target_domain*"  | wc -l)
export CONFIGEXIST
